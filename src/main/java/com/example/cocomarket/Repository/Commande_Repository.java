...
public interface Commande_Repository extends JpaRepository<Commande, Integer> {
    ...
    @Query(value ="SELECT commande_cart_id FROM commande WHERE etat like 'WAITING' GROUP BY commande_cart_id HAVING COUNT(commande_cart_id) > 3 ORDER BY COUNT(commande_cart_id) DESC", nativeQuery = true)
